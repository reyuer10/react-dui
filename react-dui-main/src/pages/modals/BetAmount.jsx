...
function BetAmount() {
  const { OpenModalConfirmationTo, amount, setAmount } =
    useContext(dealerContext);

  const handleClickAmount = (amnt) => {
    setAmount((prevAmount) => prevAmount + amnt);
  };
  ...
  return (
    <div className="font-rubik space-y-4 border-[6px] bg-yellow-500  border-zinc-800 rounded-3xl p-4 shadow-md shadow-black">
      <div className="flex space-x-4">
        <input
          ...
          disabled
          type="number"
          className=" bg-zinc-700 text-3xl p-2 font-black text-white text-center outline-none shadow-inner shadow-black border-[5px] border-zinc-700 ring-4 ring-black rounded-xl "
        />
        <button
          onClick={() => setAmount(0)}
          className="active:scale-95 transition-all duration-75 font-black text-white primary-drop-shadow text-3xl px-4 bg-yellow-400 p-2 rounded-xl shadow-inner shadow-black border-[5px] border-yellow-400 ring-4 ring-zinc-700"
        >
          <svg
      ...
      <div className="flex justify-center bg-blue-500 shadow-inner shadow-black  py-8 border-4 border-blue-500 ring-4 ring-zinc-700 rounded-2xl">
        <Keypad handleClickAmount={handleClickAmount} />
      </div>
      <div>
        <button
          onClick={handleSubmitAmount}
          className="text-white font-black text-3xl primary-drop-shadow shadow-inner shadow-black border-[5px] border-red-600 ring-4 ring-zinc-700 w-full bg-red-600 rounded-xl py-2"
        >
          BET
        </button>
      </div>
